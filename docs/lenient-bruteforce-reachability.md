...
## Reachability of brute-force rebuild from default APIs

The `Loader.loadPDF(...)` entry points construct `PDFParser` and call the no-arg `parse()` helper, which always enables lenient mode: `parse()` delegates to `parse(true)`, which calls `setLenient(true)` before any parsing occurs.【F:pdfbox/src/main/java/org/apache/pdfbox/pdfparser/PDFParser.java†L125-L170】 This means every default `Loader.loadPDF` overload runs in lenient mode unless the caller explicitly invokes `parse(false)`.

The resulting call chain to the brute-force rebuild path is:

1. `Loader.loadPDF(...)` → `new PDFParser(...)` → `parser.parse()` (lenient defaults to `true`).
2. `parse(true)` → `initialParse()` → `retrieveTrailer()`.
3. `retrieveTrailer()` attempts to locate `startxref` via `getStartxrefOffset()` and parse xref tables. If `getStartxrefOffset()` fails (no `%%EOF` / `startxref`) or xref parsing throws, lenient mode flips `rebuildTrailer` to `true`; a parsed trailer missing `/Root` also sets `rebuildTrailer` when lenient.【F:pdfbox/src/main/java/org/apache/pdfbox/pdfparser/COSParser.java†L321-L381】
4. When `rebuildTrailer` is `true`, `retrieveTrailer()` clears the xref table and invokes `getBruteForceParser().rebuildTrailer(...)`. The first call into `BruteForceParser` triggers `getBFCOSObjectOffsets()`, which in turn starts the full `bfSearchForObjects()` scan over the file body to synthesize object offsets.【F:pdfbox/src/main/java/org/apache/pdfbox/pdfparser/BruteForceParser.java†L118-L218】【F:pdfbox/src/main/java/org/apache/pdfbox/pdfparser/BruteForceParser.java†L820-L855】

### Minimal conditions to reach brute-force scanning through default APIs

* **Default leniency:** Any `Loader.loadPDF` overload uses `parse()` with `lenient = true`; no explicit opt-in is needed.
* **Triggering rebuild:** Provide a PDF without a valid `startxref`/`%%EOF`, or with a damaged xref that raises an `IOException`, or with a parsed trailer lacking `/Root`. Under lenient defaults, any of these conditions sets `rebuildTrailer = true` inside `retrieveTrailer()` and immediately routes into `BruteForceParser.bfSearchForObjects()`.
* **Result:** A malformed file opened via the standard public API can force the brute-force offset scan without the caller ever opting into lenient parsing.
